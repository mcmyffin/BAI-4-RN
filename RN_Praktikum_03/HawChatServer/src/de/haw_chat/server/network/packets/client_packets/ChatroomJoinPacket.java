...
public class ChatroomJoinPacket extends AbstractClientPacket {
    ...
    public ChatroomJoinPacket(ClientThread clientThread, String messageString) throws IllegalArgumentPacketException {
        super(clientThread);

        String[] splitedMessageString = messageString.split(" ");
        if(splitedMessageString.length < 2 || splitedMessageString.length > 3) throw new IllegalArgumentPacketException(messageString);
